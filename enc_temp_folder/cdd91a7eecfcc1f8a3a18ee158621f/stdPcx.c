...
{
	if (!bitmap || !bitmap->mipSurfaces)
		return 0;

	stdVBuffer* vbuf = *bitmap->mipSurfaces;
	...
	*(uint16_t*)&pcxHeader.reserved_4A[52] = 0;
	...
	// Open file
	stdFile_t fd = std_pHS->fileOpen(fpath, "wb");
	if (!fd)
		return 0;

	// Write header
	std_pHS->fileWrite(fd, &pcxHeader, sizeof(pcxHeader));
	...
	for (int y = 0; y < height; y++)
	{
		...
		while (x < width)
		{
			uint8_t value = pixels[offset + x];
			int count = 1;

			// run detection (max 63, stop at row boundary)
			while (x + count < width && count < 63 &&
					pixels[offset + x + count] == value)
			{
				count++;
			}
	...
	// Palette (PCX requires 0x0C + 768 bytes)
	stdFPutc(0x0C, fd);
